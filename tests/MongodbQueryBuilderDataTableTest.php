...
class MongodbQueryBuilderDataTableTest extends TestCase
{
    /** @test */
    ...
    protected function setUp()
    {
        ...
        $this->app['router']->get('/queryBuilder/users', function () {
            return datatables(DB::table('users'))->make('true');
        });

        $this->app['router']->get('/queryBuilder/addColumn', function () {
            return datatables(DB::table('users'))
                             ->addColumn('foo', 'bar')
                             ->make('true');
        });

        $this->app['router']->get('/queryBuilder/indexColumn', function () {
            return datatables(DB::table('users'))
                             ->addIndexColumn()
                             ->make('true');
        });

        $this->app['router']->get('/queryBuilder/filterColumn', function () {
            return datatables(DB::table('users'))
                             ->addColumn('foo', 'bar')
