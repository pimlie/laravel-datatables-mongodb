 ...
 */
class MongodbQueryBuilderEngine extends BaseEngine
{
    /**
    ...
    {
        return $column; //$this->connection->getQueryGrammar()->wrap($column);
    }
    ...
    protected function compileQuerySearch($query, $column, $keyword, $relation = 'or')
    {
        ...
        $query->{$relation . 'Where'}($column, 'regex', $value);
    }
    ...
    protected function prepareKeyword($keyword)
    {
        if ($this->isSmartSearch()) {
            $keyword = '/.*'.$keyword.'.*/' . ($this->isCaseInsensitive() ? 'i' : '');
        } else {
            $keyword = '/^'.$keyword.'$/' . ($this->isCaseInsensitive() ? 'i' : '');
        }
    ...
    {
        $this->query->skip((int)$this->request->input('start'))
                    ->take((int) $this->request->input('length') > 0 ? (int)$this->request->input('length') : 10);
    }
